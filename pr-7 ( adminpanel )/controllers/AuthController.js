...
const registerUser = async (req, res) => {
    try {
        ...
        if (password === cpassword) {
            ...
            console.log(`TOUR PASSWORD AND CANFORM DOSE NOT MATCHED `);
...
const forgotPassword = async (req, res) => {
    try {
        ...
        var transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
                user: 'vorakeyur562@gmail.com',
                pass: 'otur eegq omrl kvga'
            }
        });

        var mailOptions = {
            from: 'vorakeyur562@gmail.com',
            to: useremail,
            subject: 'Forgot password',
            html: `<h1 style='color:blue'>Here is ${user?.name} Your Otp :- ${otp}</h1>`
        };
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
                ...
                console.log('email has been sant: ' + info.response);
                let auth = {
                    email: useremail,
                    otp: otp
                }
